 ...
 * private key into PSA Crypto as a VOLATILE key pair with SIGN usage,
 * re-imported on every boot.
 *
 * TO USE YOUR OWN KEY:
 *   1. openssl ecparam -name prime256v1 -genkey -noout -out pox_priv.pem
 *      openssl ec -in pox_priv.pem -text -noout
 *   2. Copy the 32 hex bytes of the printed `priv:` block (drop the
 *      leading 00 if openssl prints 33) into POX_HARDCODED_PRIV_KEY.
 *   3. Keep the matching public key for your verifier.
 *
 * PRODUCTION SAFETY GUARD: while POX_HARDCODED_PRIV_KEY still holds the
 * placeholder bytes, registration is refused unless
 * POX_ALLOW_PLACEHOLDER_KEY is enabled — an option only visible when
 * TFM_BUILD_TYPE_DEBUG is set, so it cannot be left on in a release.
 */
...
/* Known-public placeholder bytes shipped in this repo; a byte-for-byte
 * match is refused unless POX_ALLOW_PLACEHOLDER_KEY is set. */
...
/* ============================ EDIT BELOW ============================ */
/* Your own 32-byte ECDSA P-256 private scalar (see file header). */
...
/* Compared against POX_HARDCODED_PRIV_KEY by the runtime guard in
 * pox_register_signing_key(). */
 ...
 * the handle via psa_import_key()'s imported_id output, stored here so
 * sign_pox_token() can use the real value.
 */
...
/* Import a 32-byte P-256 scalar as a volatile key. */
static psa_status_t import_pox_key(const uint8_t *priv, size_t priv_len)
{
    ...
    /* VOLATILE: lives only for this boot. Do NOT call psa_set_key_id()
     * — ignored for volatile keys; the handle comes via imported_id. */
...
/* One-shot registration from pox_init(). */
psa_status_t pox_register_signing_key(void)
{
    /* Fail closed on the known-public placeholder key. */
